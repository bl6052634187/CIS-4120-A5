...
function LatexEditor({ currentUser }) {
 ...
 if (isFullDocument && window.latexjs) {
 try {
 const generator = new window.latexjs.HtmlGenerator({ hyphenate: false });
 window.latexjs.parse(trimmed, { generator });
...
<html>
 ...
 applyToSelection((selected) => `\\colorbox{${highlightColor}}{${selected}}`);
 };

 const applyTextColor = () => {
 applyToSelection((selected) => `\\textcolor{${textColor}}{${selected}}`);
 };
 ...
 { label: "Quote Block", code: "\\begin{quote}\nYour quoted text here.\n\\end{quote}" },
 { label: "Centered Text", code: "\\begin{center}\nCentered text here.\n\\end{center}" },
 ]
 },
 {
 name: "Calculus",
 ...
 {category.name === "Text Formatting" && (
 <div style={{ marginTop: "0.75rem" }}>
 <div style={{ ...colorControlsRowStyle, marginTop: 0 }}>
 <div style={colorGroupStyle}>
 <span style={colorLabelStyle}>Highlight</span>
 <input
 type="color"
